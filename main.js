...
let nextTabId = 0;
let uiOffsetHeight = 0; // UI要素の合計高さを格納する変数
...
function switchTab(tabId) {
  if (views.has(tabId)) {
    ...
    const [width, height] = mainWindow.getSize();
    views.get(tabId).setBounds({ x: 0, y: uiOffsetHeight, width: width, height: height - uiOffsetHeight });
    sendNavState();
...
function createWindow() {
  ...
  mainWindow.on('resize', () => {
    if (activeViewId !== null && views.has(activeViewId)) {
      const [width, height] = mainWindow.getSize();
      views.get(activeViewId).setBounds({ x: 0, y: 120, width: width, height: height - 120 });
    }
  });
  ...
  ipcMain.on('set-ui-height', (event, height) => {
    ...
    console.log(`main.js: Received UI height: ${uiOffsetHeight}px`);
    // UIの高さが更新されたら、アクティブなBrowserViewのサイズを再調整
    if (activeViewId !== null && views.has(activeViewId)) {
      const [width, height] = mainWindow.getSize();
      views.get(activeViewId).setBounds({ x: 0, y: uiOffsetHeight, width: width, height: height - uiOffsetHeight });
    }
  });
